...
public class Raytracer {
    ...
    private Vector3 castRay(Scene scene, Ray ray) {
        ...
        if (closestObjectHit != null) {
            ...
            shadingColor = pygment.color.mult(material.ambientCoefficient);
            ...
            for (Light light : scene.lights) {
                ...
                //   um Ray, ~4 linhas)
                Vector3 vetoratefonteluz =  light.position.diff(closestIntersection.intersectionPoint).normalized();
                Ray raioSombra = new Ray(closestIntersection.intersectionPoint, vetoratefonteluz);
                double distanciaLuz = vetoratefonteluz.norm();
            ...
            for (Object obj : scene.objects) {
                RayResponse response = obj.intersectsWith(raioSombra);
                ...
                if (!hitsAnotherObjectBeforeLight) {
                    ...
                    Vector3 corluzatenuada = new Vector3();
                    Vector3 cordaluzdifusa = new Vector3();
                    corluzatenuada = light.position.mult(1/light.constantAttenuation+(distanciaLuz*light.linearAttenuation)+(distanciaLuz*distanciaLuz*light.quadraticAttenuation));
                    
                    cordaluzdifusa = material.diffuseCoefficient * pygment.color* Math.max(0, vetoratefonteluz.dotProduct());
